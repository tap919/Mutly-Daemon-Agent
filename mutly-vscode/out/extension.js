...
const reviewProviders_1 = require("./reviewProviders");
...
function getDaemonPort() {
    ...
    return userConfig.get('daemonPort') ?? DEFAULT_DAEMON_PORT;
}
let daemonClient;
let activated = false;
function activate(context) {
    ...
    daemonClient = new daemonClient_1.DaemonClient();
    function getApiKey() {
        const userConfig = vscode.workspace.getConfiguration('mutly');
        return userConfig.get('apiKey') || '';
    }
    // ── Status Bar ──────────────────────────────────────────────────
    const statusBar = new reviewProviders_1.MutlyStatusBar(daemonClient);
    context.subscriptions.push(statusBar);
    // ── Diagnostics ─────────────────────────────────────────────────
    const diagnostics = new reviewProviders_1.MutlyDiagnostics(daemonClient);
    context.subscriptions.push(diagnostics);
    // ── CodeLens ────────────────────────────────────────────────────
    const codeLensProvider = new reviewProviders_1.MutlyCodeLensProvider(daemonClient);
    const codeLensDisposable = vscode.languages.registerCodeLensProvider({ scheme: 'file' }, codeLensProvider);
    context.subscriptions.push(codeLensDisposable);
    // ── Sidebar Webview ─────────────────────────────────────────────
    const reviewPanelProvider = new reviewPanel_1.MutlyReviewPanel(context.extensionUri, daemonClient);
    const webviewDisposable = vscode.window.registerWebviewViewProvider(reviewPanel_1.MutlyReviewPanel.viewType, reviewPanelProvider);
    ...
    const applyDiffCmd = vscode.commands.registerCommand('mutly.applyDiff', async (args) => {
        let filePath = '';
        let findContent = '';
        let replaceContent = '';
    ...
    const handler = async (request, ctx, response, token) => {
...
function checkDaemonStatus() {
    const userConfig = vscode.workspace.getConfiguration('mutly');
    const apiKey = userConfig.get('apiKey') || '';
...
function queryDaemonSession(prompt, apiKey) {
...
function parseDiffFromText(text) {
