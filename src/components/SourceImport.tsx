...
export default function SourceImport({ agentState, setActiveTab }: SourceImportProps) {
  ...
  useEffect(() => {
    if (isAnalyzing && currentProgressStep < progressLogs.length) {
      ...
      const triggerServerAnalysis = async () => {
        try {
          const res = await mutlyFetch("/api/agent/analyze", {
            method: "POST",
          ...
          if (res.ok) {
            ...
            else if (data?.analysis) setReport(data.analysis);
          }
  ...
  const analysisReport = report || agentState?.lastAnalysis;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-zinc-100 flex items-center gap-2">
                  ...
                  <span className="font-mono text-indigo-400">{analysisReport.name}</span>
                </p>
                <p className="text-xs text-zinc-500">Indexed {analysisReport.fileCount} files ({analysisReport.loc.toLocaleString()} LOC) &bull; AST complete</p>
              </div>
            </div>
            <button 
              onClick={() => handleStartAnalysis(analysisReport.type)}
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/20 p-5 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold font-mono text-red-400">{analysisReport.overloadRatio}%</span>
                </div>
                <div className="space-y-1">
                  <div className="h-2 w-full bg-zinc-850 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-yellow-500 to-red-500 rounded-full" style={{ width: `${analysisReport.overloadRatio}%` }}></div>
                  </div>
              ...
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/20 p-5 space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold font-mono text-indigo-400">-{analysisReport.tokenSavingsPotential}% Tokens</span>
                </div>
                <div className="space-y-1">
                  <div className="h-2 w-full bg-zinc-850 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-zinc-700 to-indigo-500 rounded-full" style={{ width: `${analysisReport.tokenSavingsPotential}%` }}></div>
                  </div>
                </div>
                <p className="text-xs text-zinc-400 leading-relaxed">
              ...
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/20 p-5 text-center flex flex-col justify-center items-center">
                ...
                <div className="text-3xl font-display font-light text-zinc-100 my-1">{analysisReport.complexityIndex} <span className="text-xs text-zinc-500">/ 100</span></div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/20 p-6 space-y-4">
                ...
                <div className="space-y-3">
                  {analysisReport.tree.map((task: any, index: number) => (
